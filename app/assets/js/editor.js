(function($) {
    ...
    tinymce.PluginManager.add( config.slug, function( editor, url ) {
        ...
        function visualEditorFormat( content ) 
        {
            ...
            // Replace one or more line breaks with <br> inside <p>, <pre>, and <code>
            return innerHTMLRegex( content, ['code','pre','p'], function(c) {
                // the regex /([\n\r])/g (no + sign) adds multiple line breaks, 
                // probably due to some kind of WordPress core functionality
                return c.replace(/([\n\r]+)/g, "<br />");
            });
        }
         ...
         * @param {array} tags HTML tags array
         * @param {function} replacer a function to run on the inner HTML of the element
         * @returns string
         */
        function innerHTMLRegex( content, tags, replacer )
        {
            var regex = new RegExp("<("+tags.join('|')+")([^>]*)>([\\s\\S]*?)<\\/("+tags.join('|')+")>","g");
            return content.replace( regex, function( match, tag, atts, content ) {
                return '<'+tag+atts+'>'+replacer( content )+'</'+tag+'>';
            });
        }
